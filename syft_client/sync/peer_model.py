...
class TransportEndpoint:
    ...
    endpoint_data: Dict[str, Any] = field(default_factory=dict)  # Transport-specific data
...
class Peer:
    ...
    def get_verified_transports(self) -> List[str]:
        ...
        return [name for name, endpoint in self.available_transports.items() if endpoint.verified]
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Peer':
        ...
        if data.get('capabilities_last_updated'):
            data['capabilities_last_updated'] = datetime.fromisoformat(data['capabilities_last_updated'])
                ...
                class TransportAccess:
                    ...
                    def __getattr__(self, transport_name):
                        ...
                        if not hasattr(self.platform, transport_name):
                            raise AttributeError(f"Transport '{transport_name}' not found in {self.platform_name}")
                                ...
                                try:
                                    ...
                                    if resource and hasattr(resource, '__class__') and resource.__class__.__name__ == 'PeerResource':
                                        ...
                                        if resource.available:
                                            ...
                                            if hasattr(resource, 'resource_type') and resource.resource_type != 'folders':
                                        ...
                                        if resource.get('available', True):
                                            ...
                                            # Build status line
                                            status_parts = [f"[green]✓[/green] [cyan].{transport_name}[/cyan]"]
                                            
                                            # Add resource details if available
                                            if name:
                                                # Truncate long names
                                                display_name = name[:30] + '...' if len(name) > 30 else name
                                                status_parts.append(f"[dim]({resource_type}: {display_name})[/dim]")
                                            elif resource_type != 'resource':
                                                status_parts.append(f"[dim]({resource_type})[/dim]")
                ...
                for platform_name in sorted(self._client._platforms.keys()):
                    ...
                    for transport_name in transport_access.__dir__():
                        try:
                            ...
                            if resource and isinstance(resource, dict) and resource.get('available', True):
                                resource_count += 1
                        except:
                            pass
    ...
    def __repr__(self) -> str:
        ...
        # Add metadata
        main_table.add_row("")
        main_table.add_row(f"[dim]Added: {self.added_date.strftime('%Y-%m-%d %H:%M')}[/dim]")
        if self.last_interaction:
            main_table.add_row(f"[dim]Last interaction: {self.last_interaction.strftime('%Y-%m-%d %H:%M')}[/dim]")
        if self.capabilities_last_updated:
            main_table.add_row(f"[dim]Capabilities updated: {self.capabilities_last_updated.strftime('%Y-%m-%d %H:%M')}[/dim]")
        ...
        if self.platform and hasattr(self, '_client') and self._client and self.platform in self._client._platforms:
            ...
            for transport_name in sorted(all_transports):
                if transport_name in self.available_transports:
                    ...
                    # Status indicators
                    verified = "[green]✓[/green]" if endpoint.verified else "[red]✗[/red]"
                    ...
                    transport_line = f"    {verified} [cyan].{transport_name}[/cyan]{perf_info}"
                    main_table.add_row(transport_line)
                    
                    # Show last verified time if available
                    if endpoint.verified and endpoint.last_verified:
                        time_ago = (datetime.now() - endpoint.last_verified).total_seconds()
                        if time_ago < 3600:
            ...
            for transport_name, endpoint in self.available_transports.items():
                ...
                platforms_with_transports[platform_key].append((transport_name, endpoint))
