...
class MessageProcessor:
    ...
    def process_messages(self, messages: Dict[str, List[Dict]], 
                        peer_email: str) -> Dict[str, int]:
        """
        ...
        for transport, msg_list in messages.items():
            if self.verbose:
                print(f"Processing {len(msg_list)} messages from {peer_email} via {transport}")
    ...
    def _process_archive_message(self, archive_path: str, message_id: str, 
                                peer_email: str) -> bool:
    ...
    def _process_direct_message(self, file_path: str, message_id: str, 
                               peer_email: str) -> bool:
    ...
    def _determine_target_path(self, source_path: Path, metadata: Dict) -> Path:
        ...
        if "datasite" in metadata:
            return self.syftbox_dir / "datasites" / metadata["datasite"] / source_path.name
