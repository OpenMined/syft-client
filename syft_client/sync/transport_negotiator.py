    ...
    def select_transport(
        ...
        if self.client.verbose:
            ...
            for score in scores[:3]:  # Show top 3
                print(f"   - {score.transport_name}: {score.score:.2f} (est. {score.estimated_latency_ms:.0f}ms)")
    ...
    def _score_latency(
        ...
        if requirements.requested_latency_ms:
            if estimated_latency <= requirements.requested_latency_ms:
                ...
                score = max(0, ratio)
                reasons.append(f"Latency {estimated_latency:.0f}ms vs requested {requirements.requested_latency_ms}ms")
        else:
