...
class GSheetsTransport(BaseTransportLayer, BaseTransport):
                ...
                if is_syft:
                    try:
                        result = (
                            ...
                            .values()
                            .get(
                                spreadsheetId=file["id"], range="A:Z"  # Get all columns
                            )
                            .execute()
                        )
    ...
    def _send_archive_via_transport(
        ...
        print(
            f"   - sheets_service exists: {self.sheets_service is not None}", flush=True
        )
        print(
            f"   - drive_service exists: {self.drive_service is not None}", flush=True
        )
        ...
        try:
            ...
            print(
                f"   📋 _get_or_create_message_sheet returned: {sheet_id}", flush=True
            )
            if not sheet_id:
    ...
    def _get_messages_from_transport(
        ...
        try:
            ...
            if verbose:
                ...
                if hasattr(self, "verbose") and self.verbose:
                    print(
                        f"   [{timestamp}] 📊 SHEETS API: values.get (reading messages)"
                    )
            result = (
                self.sheets_service.spreadsheets()
                .values()
