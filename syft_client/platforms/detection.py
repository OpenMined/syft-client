...
class Platform(Enum):
    ...
    def description(self):
        # Optional: add special cases for nicer formatting
        default = f"{self.name.replace('_', ' ').title()} client"
        special_cases = {
            "google_personal": "Google client",
            "google_org": "Google Workspace client",
            "microsoft": "Microsoft client",
        }
        return special_cases.get(self.name, default)

    def platform_description(self):
        if self.value in ["google_personal", "google_org"]:
            platform_desc = f"Google ({self.email})"
        elif self.value == "microsoft":
            platform_desc = f"Microsoft ({self.email})"
        else:
            platform_desc = f"{self.value} ({self.email})"
