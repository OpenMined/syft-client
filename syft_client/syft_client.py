...
from syft_client.peers.peers_property import PeersProperty
...
import io
import contextlib
from syft_client.utils.printing_utils import LoginProgressPrinter, print_progress_login
...
class SyftClient:
    ...
    def _check_peer_requests_login(self, accept_requests: bool = True):
        ...
        try:
            ...
            if hasattr(self, "sync") and hasattr(self.sync, "peers_manager"):
                ...
                peer_requests_data = self.sync.peers_manager.check_all_peer_requests(
                    verbose=False
                )
            ...
            pass
        return peer_request_output

    def _kill_servers_login(
        self, environment: Environment, verbose: bool, current_step: int
    ):
        print_progress_login(
            environment,
            verbose,
            current_step,
        ...
        try:
            ...
            print_progress_login(
                ...
                verbose,
                current_step,
                f"✓ Cleaned up {killed_count} existing server(s)",
            )
        except Exception as e:
            if verbose:
                print(f"⚠️  Error killing existing servers: {e}")

    def _start_servers_login(
        self,
        ...
        current_step += 1
        printer.print_progress_login(current_step, "Starting watcher")
        ...
        current_step += 1
        printer.print_progress_login(current_step, "Starting receiver")

        self.start_receiver()

        current_step += 1
        printer.print_progress_login(current_step, "Starting job runner")
    ...
    def get_active_transports(self) -> List[str]:
        active_transports = []
        for _, platform in self._platforms.items():
    ...
    def _create_local_syftbox_directory_login(self, verbose: bool):
        if verbose:
            f = io.StringIO()
            with contextlib.redirect_stdout(f):
    ...
    def _clear_peer_caches_login(self, verbose: bool):
        if verbose:
            f = io.StringIO()
            with contextlib.redirect_stdout(f):
                try:
    ...
    def _login(
        ...
        environment = detect_environment()
        printer = LoginProgressPrinter(environment, verbose, 14)

        current_step = 1
        current_step = printer.print_progress_login(
            current_step, f"Starting login for {self.email}"
        )

        current_step = printer.print_progress_login(
            current_step, "Detecting email platform"
        )

        platform = detect_primary_platform(self.email, provider)

        current_step = printer.print_progress_login(
            current_step, "Detecting environment"
        )

        try:
            # Step 4: Create platform client
            current_step = printer.print_progress_login(
                current_step, f"Initializing {platform.description}"
            )
            ...
            current_step = printer.print_progress_login(
                current_step, f"Authenticating via {platform.platform_description()}"
            )
            auth_result = client.authenticate()
            self.add_platform(client, auth_result)

            current_step = printer.print_progress_login(
                current_step, "Setting up local SyftBox directory"
            )

            self._create_local_syftbox_directory_login(verbose)

            self._clear_peer_caches_login(verbose)

            current_step = printer.print_progress_login(
                current_step, "Activating peer-to-peer channels"
            )
            if init_transport:
                self._initialize_all_transports()

            current_step = printer.print_progress_login(
                current_step, "Checking for peer requests"
            )

            peer_request_output = self._check_peer_requests_login(accept_requests)

            # Kill existing servers if requested (do this even if skip_server_setup)
            verbose = True
            if kill_servers:
                self._kill_servers_login(environment, verbose, current_step)

            if not skip_server_setup:
                current_step = self._start_servers_login(
                    environment, verbose, current_step, printer, kill_servers
                )

            # Step 9: Warm the cache
            current_step = printer.print_progress_login(
                current_step, "Getting list of active transports"
            )

            # Final success message with peer count
            printer.print_final_message(self.get_active_transports(), self.peer_count)
