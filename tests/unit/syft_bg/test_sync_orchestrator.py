...
def _make_orchestrator(temp_dir, **config_overrides):
    ...
    orch = SyncOrchestrator(client, scanner, writer, config)
    return orch, client, scanner


class TestRunOnce:
    def test_syncs_and_writes_snapshot(self, temp_dir):
        orch, client, _ = _make_orchestrator(temp_dir)
        orch.run_once()
    ...
    def test_captures_inbox_messages(self, temp_dir):
        orch, _, scanner = _make_orchestrator(temp_dir)
        scanner.scan_inbox_messages.return_value = [
    ...
    def test_captures_drive_peers(self, temp_dir):
        orch, _, scanner = _make_orchestrator(temp_dir)
        scanner.scan_peer_emails.return_value = ["ds@test.com"]
        scanner.scan_approved_peers.return_value = ["ds@test.com"]
        orch.run_once()
        snap = SnapshotReader(temp_dir / "snapshot.json").read()
        assert "ds@test.com" in snap.drive_peer_emails
        assert "ds@test.com" in snap.drive_approved_peers
...
class TestSyncCount:
    def test_increments_across_cycles(self, temp_dir):
        orch, _, _ = _make_orchestrator(temp_dir)
        orch.run_once()
        orch.run_once()
        snap = SnapshotReader(temp_dir / "snapshot.json").read()
...
class TestNoScanner:
    def test_works_without_inbox_scanner(self, temp_dir):
        client = MagicMock()
        client.job_client.jobs = []
        client.peer_manager.approved_peers = []
        writer = SnapshotWriter(temp_dir / "snapshot.json")
